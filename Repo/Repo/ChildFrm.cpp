...
BOOL CChildFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CLeftView), CSize(100, 100), pContext) ||
		!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CRepoView), CSize(100, 100), pContext))
	{
...
CRepoView* CChildFrame::GetRightPane()
{
	CWnd* pWnd = m_wndSplitter.GetPane(0, 1);
	CRepoView* pView = DYNAMIC_DOWNCAST(CRepoView, pWnd);
	return pView;
}
...
void CChildFrame::OnViewStyle(UINT nCommandID)
{
	...
	// if the right-hand pane has been created and is a CRepoView, process the menu commands...
	if (pView != NULL)
	{
		int nStyle = -1;
